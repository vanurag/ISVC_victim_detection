...
\algnewcommand{\LineComment}[1]{\State \textcolor{blue}{\(\#\) #1}}
...
\State $I_{IR} \gets \texttt{getImage}(message)$\Comment{Read the image.}
\EndIf
\LineComment{Read ROS grayscale image message.}
\If{incoming message topic $==$ grayscale image topic}
\State $I_{G} \gets \texttt{getImage}(message)$\Comment{Read the image.}
...
\EndIf
\LineComment{Remap the ROIs into rectified image and extract overlapping patches in both the images corresponding to the new ROIs.}
